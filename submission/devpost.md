...
| **Feature creep** | 76 checks on a rubric with no box for them | Cut them. The suite that stayed is tied to the **demo path**: 14 then, **36 now** — every addition binds a specific claim in the README to a specific line of script output, so it grows only when a claim does |
